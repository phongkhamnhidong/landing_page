...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const [cat, posts, total] = await Promise.all([
    ...
    isSearching
      ? client.fetch(postsByCategorySearchQuery, { categorySlug: category, q: searchTerm })
      : client.fetch(postsByCategoryQuery, { categorySlug: category, from, to }),
    isSearching
      ? Promise.resolve(0)
      : client.fetch(postsByCategoryCountQuery, { categorySlug: category }),
  ])
  ...
  function pageHref(p: number) {
    ...
    return `/kien-thuc/${category}${qs ? `?${qs}` : ""}`
  }
