...
import {
  tinTucCategoriesQuery,
  allTinTucPageQuery,
  allTinTucCountQuery,
  tinTucSearchPageQuery,
  tinTucSearchCountQuery,
...
export default async function TinTucPage({ searchParams }: Props) {
  ...
  const [categories, posts, total] = await Promise.all([
    ...
    isSearching
      ? client.fetch(tinTucSearchPageQuery, { q: searchTerm, from, to })
      : client.fetch(allTinTucPageQuery, { from, to }),
    isSearching
      ? client.fetch(tinTucSearchCountQuery, { q: searchTerm })
      : client.fetch(allTinTucCountQuery),
  ])
